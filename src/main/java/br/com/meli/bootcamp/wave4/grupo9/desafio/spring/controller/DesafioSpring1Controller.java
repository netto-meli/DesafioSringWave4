...
public class DesafioSpring1Controller {
	...
	public ResponseEntity<ProdutoDTO> obter(@PathVariable Long id) {
		return ResponseEntity.ok(ProdutoDTO.converteProdutoparaProdDto(produtoService.obter(id)));
	}

	@GetMapping("/produto")
	private ResponseEntity<List<ProdutoDTO>> getList() {
		//estoque.teste();
		List<Produto> listaProduto = produtoService.lista();
		return ResponseEntity.ok(
				ProdutoDTO.
				converterListaProdParaListaDTO(listaProduto));
	}
}
